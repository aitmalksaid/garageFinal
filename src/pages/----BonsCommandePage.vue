  ...
  try {
    ...
    for (const bon of bonsCommande.value) {
      try {
        ...
        // Calculer le progrès
        const progress = await dataService.getBonCommandeProgress(bon.id, 4) // 4 = statut_id terminé
...
const updateArticleStatus = async (articleId, newStatus) => {
  try {
    const updateData = { statut_id: parseInt(newStatus) }
    if (newStatus === 'recu') {
      updateData.date_reception = new Date().toISOString().split('T')[0]
    }
    await dataService.updateBonCommandeArticle(articleId, updateData)
    
    // Recalculer le progrès après la mise à jour du statut de l'article
    try {
      const bonId = bonArticles.value[articleId]?.bon_commande_id
      if (bonId) {
        const progress = await dataService.getBonCommandeProgress(bonId, 4) // 4 = statut_id terminé
        bonProgress.value[bonId] = progress
      }
    } catch (progressError) {
      console.error('Erreur lors du recalcul du progrès:', progressError)
    }
